...
from reports import generate_rebalance_report, save_portfolio_state_csv
...
def plan_orders(orders, *, stage: str) -> List[Dict[str, Any]]:
    ...
    for order in orders:
        ...
        reason = getattr(order, "reason", "") or stage
        entry = {
            "stage": stage,
            "reason": reason,
            "side": side,
        ...
        logger.info(
            "Planned order (not submitted): stage=%s reason=%s side=%s ticker=%s qty=%s",
            stage,
            reason,
            side,
            order.ticker,
...
def _format_telegram_message(
    *,
    ...
    regime_ticker = html.escape(str(regime_summary.get("ticker", "^GSPC")))
...
def _humanize_order_reason(reason: object, *, stage: object = None) -> str:
    raw_reason = str(reason or "").strip()
    raw_stage = str(stage or "").strip()
    key = raw_reason or raw_stage
    if not key:
        return "n/a"
    ...
    labels = {
        ...
        "new_buy": "new buy",
        "initial_sell": "sell dropout",
        "ranked_position > top_n": "ranking > top_n",
        "below_sma": "price < SMA100",
        "gap_exceeded": "gap >= 15% in 90d",
        "share_class_excluded": "share class excluded",
        "no_data": "missing market data",
        "no_momentum_data": "missing momentum window",
        "nan_close": "NaN close data",
        "momentum_failed": "momentum regression failed",
        "atr_missing": "ATR20 unavailable",
        "missing_close": "missing close data",
        "sma_missing": "SMA100 unavailable",
        "missing_ticker": "missing ticker",
        "missing_base_symbol": "missing base symbol",
    }
...
def _format_order_table(order_results: Sequence[Dict[str, Any]], name_map: Dict[str, str]) -> str:
    ...
    for order in order_results:
        ...
        reason_text = _humanize_order_reason(order.get("reason"), stage=order.get("stage"))
        rows.append((orb, str(ticker), str(name), qty_text, reason_text))
    ...
    reason_width = max(len(header_reason), *(len(row[4]) for row in rows)) if rows else len(header_reason)

    header = (
        f"{header_ticker:<{ticker_width}}  {header_company:<{company_width}}  "
        f"{header_qty:>{qty_width}}  {header_reason:<{reason_width}}"
    )
    ...
    for orb, ticker, company, qty, reason in rows:
        lines.append(
            f"{orb} {ticker:<{ticker_width}}  {company:<{company_width}}  {qty:>{qty_width}}  {reason:<{reason_width}}"
        )
    return "\n".join(lines)
...
def _format_pending_positions_blocks(
    ...
    *,
    ...
    for order in order_results:
        ...
        reason = _humanize_order_reason(order.get("reason"), stage=stage)
        name = name_map.get(str(ticker), "")
        planned_rows.append(f"{side_icon} {ticker}, {name}, {_format_qty(qty)}, reason={reason}")
    if not planned_rows:
        lines.append("None.")
    else:
