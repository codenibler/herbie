...
def build_sell_orders(
    ...
    *,
    ...
    for position in positions:
        ...
        orders.append(OrderRequest(ticker=ticker, quantity=-qty, reason=reason))
...
def build_rebalance_orders(
    ...
    *,
    ...
    for plan in plans:
        ...
        sell_orders.append(OrderRequest(ticker=stock.ticker, quantity=-sell_qty, reason="rebalance_sell"))
    ...
    for plan in plans:
        ...
        rebalance_buy_orders.append(OrderRequest(ticker=stock.ticker, quantity=delta_qty, reason="rebalance_buy"))
    ...
    for plan in plans:
        ...
        if delta_qty <= 0:
            break
        delta_qty = round(delta_qty, 3)
        new_buy_orders.append(OrderRequest(ticker=stock.ticker, quantity=delta_qty, reason="new_buy"))
        remaining_cash -= required_cash
        gap_text = "n/a"
        if stock.max_gap_percent is not None:
            gap_text = f"{stock.max_gap_percent:.3f}%"
